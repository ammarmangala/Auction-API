...
namespace Auction_API.Services
{
    public class AuctionItemService : IAuctionItemService
    {
        ...
        public async Task<AuctionItem> CreateAuctionItem(CreateAuctionItemDTO dto, string userId)
        {
            ...
            {
                // Als het een Gold of Platinum gebruiker is, kan de einddatum vrij gekozen worden, maar moet minimaal 12 uur na de startdatum zijn
                endDateTime = dto.EndDateTime > dto.StartDateTime.AddHours(12)
                ? dto.EndDateTime
                : dto.StartDateTime.AddHours(12);
            }
            else
            {
